...
export default function AddPartTypeModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createPartType(dataToSubmit)
      toast.success('Part type created successfully')
      onSuccess()
      onClose()
    } catch (error) {
      console.error('Error creating part type:', error)
      toast.error(error.response?.data?.detail || 'Failed to create part type')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-4 rounded-t-lg flex justify-between items-center border-b border-gray-700">
        <h3 className="text-xl font-medium text-white">Add New Part Type</h3>
